  ...
  return (
    <>
      ...
      <div className="fixed top-4 z-50 w-full px-4 md:top-6">
        {/* Narrower, content-hugging shell (not the full 1280px page container). */}
        <nav className="glass-panel mx-auto w-full max-w-4xl rounded-control shadow-xl">
            ...
            <div className="hidden gap-2 md:flex">
              {navItems.map((item) => (
                <a
                  key={item.href}
                  href={item.href}
                  className="rounded-control px-4 py-2 text-body-md text-on-surface-variant transition-all hover:bg-on-surface/5 hover:text-primary-container"
                >
                  {item.label}
                </a>
              ))}
          ...
          {isOpen && (
            <div className="flex flex-col gap-1 rounded-b-control border-t border-outline-variant/30 bg-surface px-6 py-4 md:hidden">
              {navItems.map((item) => (
                <a
                  key={item.href}
                  href={item.href}
                  onClick={closeMenu}
                  className="rounded-control px-3 py-3 text-body-md text-on-surface-variant transition-colors hover:bg-on-surface/5 hover:text-primary-container"
                >
                  {item.label}
                </a>
              ))}
